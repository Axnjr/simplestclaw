 ...
 *   GOOGLE_API_KEY       - Legacy alias for GEMINI_API_KEY (supported)
 ...
 *   OPENCLAW_PROVIDER      - Preferred provider for unqualified MODEL values (anthropic|openai|google|gemini|openrouter)
 *   MODEL                  - Default model override (e.g. "google/gemini-3-pro-preview")
...
/**
 * Supported provider authentication env vars and default models.
 * NOTE: OpenClaw resolves Google auth via GEMINI_API_KEY; GOOGLE_API_KEY is a backward-compatible alias.
 */
const PROVIDERS = [
  {
    id: 'anthropic',
...
/**
 * Keep Google/Gemini env keys in sync for compatibility across OpenClaw versions.
 */
function normalizeGeminiApiKeyAliases() {
...
/**
 * Returns configured provider IDs in deterministic priority order.
 *
 * @returns {string[]}
 */
function getConfiguredProviderIds() {
  return PROVIDERS.filter((provider) =>
    provider.envVars.some((envVar) => process.env[envVar]?.trim())
  ).map((provider) => provider.id);
}

/**
 * Resolve preferred provider from OPENCLAW_PROVIDER (if set) or configured API keys.
 *
 * @returns {string|null}
 */
function resolvePreferredProviderId() {
  const requestedProvider = process.env.OPENCLAW_PROVIDER?.trim().toLowerCase();
  if (requestedProvider) {
    const normalized = requestedProvider === 'gemini' ? 'google' : requestedProvider;
    const provider = PROVIDERS.find((entry) => entry.id === normalized);
    if (provider && provider.envVars.some((envVar) => process.env[envVar]?.trim())) {
      return provider.id;
    }
  }
  return getConfiguredProviderIds()[0] || null;
}

/**
 * Pick the default model to use on first run.
 * Priority:
 *  1) Explicit MODEL env var
 *  2) Provider-specific default based on configured API keys
 *
 * @returns {string|null}
 */
function resolveInitialModel() {
  const explicitModel = process.env.MODEL?.trim();
  const selectedProvider = resolvePreferredProviderId();
  ...
  if (explicitModel) {
    ...
    if (selectedProvider) {
      return `${selectedProvider}/${explicitModel}`;
    }
    return explicitModel;
  }
  ...
  return PROVIDERS.find((provider) => provider.id === selectedProvider)?.defaultModel || null;
}

/**
 * Resolve where OpenClaw expects its config file.
 *
 * @returns {string}
 */
...
/**
 * Bootstrap a minimal config on first run so non-Anthropic providers work out of the box.
 * If the file already exists, we leave user config untouched.
 */
function ensureOpenClawInitialConfig() {
  ...
  console.log(`Created OpenClaw config at ${configPath} (default model: ${initialModel})`);
}
...
function validateEnvironment() {
  normalizeGeminiApiKeyAliases();
  configuredProviderIds = getConfiguredProviderIds();
  hasApiKey = configuredProviderIds.length > 0;
  ...
  if (!hasApiKey) {
    ...
    console.error('║  • GEMINI_API_KEY    (from aistudio.google.com)             ║');
    console.error('║    (GOOGLE_API_KEY also works as a legacy alias)            ║');
  ...
  const configuredProviders = configuredProviderIds.map((providerId) =>
  ...
  if (!hasApiKey) {
    ...
    statusDetails = `
      ...
      <ul>
        ...
        <li><code>GEMINI_API_KEY</code> - from <a href="https://aistudio.google.com" target="_blank">aistudio.google.com</a> (<code>GOOGLE_API_KEY</code> also works)</li>
...
// Keep GOOGLE_API_KEY/GEMINI_API_KEY in sync and create a first-run config
// so provider-specific deployments (Gemini/OpenAI/OpenRouter) work immediately.
normalizeGeminiApiKeyAliases();
ensureOpenClawInitialConfig();
